...
export class XlsxImportYargsBuilder extends CommonImporttYargsBuilder {
  ...
  addWorksheetNrOption() {
    this.y = this.y.options('worksheetNr', {
      describe: 'Index of the worksheet to import (default: 0)',
      type: 'number',
      default: 0,
    });
    return this;
...
async function xlsx_2_translation_objects(argv: XLSXImportArguments) {
  ...
  const worksheetNr = argv.worksheetNr;

  // The file generated by export to_xlsx has a single worksheet & I only care on
  let worksheet = workbook.worksheets[worksheetNr];
