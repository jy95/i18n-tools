          ...
          // Second format of settings.js (keySeparator)
          {
            ...
            contents: `module.exports = {
              ...
              "columns": "${path
                ...
                .replace(/\\/g, '\\\\')}",
                "worksheetName": "Settings 5 - Worksheet",
                "filename": 'settings5-output',
                "outputDir": "${TEMP_FOLDER.replace(/\\/g, '\\\\')}",
                "keySeparator": false
              }
            `,
...
describe('[export_xlsx command]', () => {
  describe('Check command availability', () => {
    ...
    it('Should display to_xlsx help output', async () => {
      const output = await fetchOutput(parser)('export to_xlsx --help');
  ...
  describe('E2E successful scenarios', () => {
    ...
    test.each([
      ...
      ['settings.js (keySeparator set to false)', TEST_FILE_SETTINGS5],
